...
$errorBusqueda = false;
if(empty($busqueda)){
    $stmt = $conexion->prepare("SELECT COUNT(*) as totalRegistro FROM servicios WHERE idCategoria = 3");
    $stmt->execute();
   ...
   //No anda la pasada por parametros del limit
    ...
    $stmt = $conexion->prepare("SELECT DISTINCT nombreServicio, imgUsuario,urlFoto,idServicio FROM servicios INNER JOIN usuarios ON usuarios.idUsuario = servicios.idUsuario INNER JOIN vehiculos ON vehiculos.idVehiculo = servicios.idVehiculo INNER JOIN fotos_vehiculo ON fotos_vehiculo.idVehiculo = vehiculos.idVehiculo WHERE nombreServicio LIKE :busqueda AND idCategoria = 3 GROUP BY idServicio;");
